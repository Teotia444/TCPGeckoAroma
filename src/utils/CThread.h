...
public:
    ...
    CThread(int32_t iAttr, int32_t iPriority = 16, int32_t iStackSize = 0x8000, CThread::Callback callback = NULL, void *callbackArg = NULL)
        ...
        if(pThread && pThreadStack) {
            OSCreateThread(pThread, &CThread::threadCallback, 1, (char*) this, (void *)(pThreadStack+iStackSize), iStackSize, iPriority, iAttributes);
        }
    }
    ...
    static CThread *create(CThread::Callback callback, void *callbackArg, int iAttr = OS_THREAD_ATTRIB_AFFINITY_ANY, int iPriority = 16, int iStackSize = 0x8000) {
    ...
    virtual void shutdownThread(void) {
        ...
        if(pThread && !(iAttributes & OS_THREAD_ATTRIB_DETACHED)) {
...
private:
    static int32_t threadCallback(int argc, const char **arg) {
